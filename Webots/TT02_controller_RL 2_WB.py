...
class WebotsGymEnvironment(Driver, gym.Env):
		...
		try :
			...
			for i in range(260,360) :
				if (donnees_lidar_brutes[-i]>0) and (donnees_lidar_brutes[-i]<12) :
					...
					tableau_lidar_mm[i] = 0
		except : # when the lidar has the problem, 
			self.nb_pb_lidar+=1
	...
	# Step function, for the action of the next step
...
def main():
	...
	model = PPO(policy="MultiInputPolicy",
		env=env, 
		learning_rate=3e-4,
		verbose=1,# pour afficher les informations
		device='cpu',
		batch_size=64, # Factor of n_steps
		n_steps=4096,
		clip_range=0.15,
		n_epochs=40,# Number of policy updates per iteration of n_steps
		tensorboard_log='./PPO2_Tensorboard',)
        # parametres par default
		# gamma=0.99,# coefficiant d'actualisation
		# gae_lambda=0.95,
		# clip_range=0.2,
		# vf_coef=1,
		# ent_coef=0.01)
	...
	#model.set_env(env)
	# Training
	#callback = LearningRateCallback(model,decay_rate=0.99)
